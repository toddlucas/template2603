...
This guide walks through enabling Row-Level Security (RLS) for the WarehouseDbContext when you start adding domain entities (Contacts, Sequences, Campaigns, etc.).

## Current State

✅ **Ready for RLS:**
- ✅ Guard registered with keyed services: `nameof(WarehouseDbContext)`
- ✅ Middleware resolves WarehouseDbContext guard
- ✅ Attributes support `[TenantRead(nameof(WarehouseDbContext))]`
- ✅ RlsPolicyManager structure in place
- ✅ Infrastructure fully implemented

⏸️ **Not Yet Enabled:**
- ⏸️ No Warehouse entities exist yet (WarehouseDbContext is empty)
- ⏸️ No tables registered in RlsPolicyManager
- ⏸️ Tenant interceptor disabled for WarehouseDbContext
- ⏸️ No RLS migration created yet

## Step-by-Step Enablement

### Step 1: Create Your First Warehouse Entity

When you create your first Warehouse entity (e.g., Contact), ensure it has `TenantId`:
...
### Step 2: Add DbSet to WarehouseDbContext
...
cd main/src/server/Base2.Data.Npgsql

# Create migration
dotnet ef migrations add AddContactEntity --context WarehouseDbContext
```
...
dotnet ef migrations add EnableRlsForWarehouse --context WarehouseDbContext
```
...
public partial class EnableRlsForWarehouse : Migration
{
...
### Step 6: Enable Tenant Interceptor

In `IServiceCollectionExtensions.cs`, update `AddWarehouseDbConfiguration`:
    ...
    IConfiguration configuration)
{
    ...
    // ✅ Enable interceptor for WarehouseDbContext
    ...
    return services.AddProviderDbContext<WarehouseDbContext>(
        configuration, 
        "WarehouseDb", 
        "Npgsql", 
...
public async Task Contact_ShouldBeIsolatedByTenant()
{
    ...
    using (var scope = container.BeginScope())
    {
        scope.Warehouse.Contacts.AddRange(
    ...
    using (var scope = container.BeginScope())
    {
        var allContacts = await scope.Warehouse.Contacts.ToArrayAsync();
...
- [ ] Controller uses `[TenantRead(nameof(WarehouseDbContext))]` or `[TenantWrite(nameof(WarehouseDbContext))]`
...
public async Task<ActionResult> GetContactWithOwner(Guid contactId)
{
    ...
    var contact = await _contactService.ReadAsync(contactId);  // WarehouseDb
    var owner = await _userService.ReadAsync(contact.OwnerId);  // AppDb
...
dotnet ef migrations list --context WarehouseDbContext
...
- [Multi-Database RLS Usage](../../guides/server/multi-database-rls-usage.md) - Usage patterns
- [RLS Patterns](./rls-patterns.md) - RLS fundamentals  
- [Database Architecture](./database-architecture.md) - Control vs data plane
- [Keyed Services Injection Guide](../../guides/server/keyed-services-injection-guide.md) - Guard injection
